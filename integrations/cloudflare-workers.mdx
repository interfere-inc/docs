...
Connecting Cloudflare lets Interfere follow your Worker deployments and tie each version to a release. Connect it from **Settings → Integrations**.
...
| `offline-access` | Get a refresh token so the connection stays alive without re-auth. |
