...
<Check>
  That's the whole install. Run your app, trigger an error, and it shows up in your dashboard within seconds, already grouped and triaged. There's nothing else to configure. (The SDK stays quiet outside production; see [the FAQ](#faq) to capture in development.)
</Check>
...
<CardGroup cols={3}>
  <Card title="Vite + React" icon="/images/logos/vite/original.svg">
    `@interfere/vite`: a Vite plugin plus `init()` before render. Works for SPA and SSR.
  </Card>

  <Card title="TanStack Start" icon="/images/logos/tanstack/original.svg">
