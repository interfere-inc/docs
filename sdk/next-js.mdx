...
### Coexisting with other source-map uploaders

Sentry (`@sentry/nextjs`) and PostHog (`@posthog/nextjs-config`) read the same `.next` output and also delete maps after upload by default. When more than one uploader runs, the first to delete starves every later one. The rule: only the **last** uploader to run deletes; every earlier one keeps its maps.

| Uploader  | Deletion flag                            | Default | Keep maps for a later uploader |
| --------- | ---------------------------------------- | ------- | ------------------------------ |
...
Make Interfere the last uploader, so it purges while Sentry keeps its maps:
...
If another tool runs last instead, invert it: set `sourceMaps: { purge: false }` on Interfere and leave that tool's deletion flag on. Un-symbolicated traces after a deploy mean an earlier uploader deleted the maps too soon — flip the flags so the purging plugin is genuinely last.
