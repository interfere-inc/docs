...
<Check>
  That's the whole install for a single-page app. Run it, trigger an error, and it shows up in your dashboard within seconds, already grouped and triaged. (To control whether the SDK runs in development, see [the FAQ](#faq).)
</Check>
...
<CardGroup cols={2}>
  <Card title="Next.js" icon="/images/logos/nextjs/original.svg" href="/sdk/next-js">
    `@interfere/next`: a build plugin, the ingest route, and a provider for the App Router.
  </Card>

  <Card title="NestJS" icon="/images/logos/nestjs/original.svg">
...
<AccordionGroup>
  ...
  <Accordion title="Some telemetry is blocked by ad-blockers on my SPA.">
    A pure single-page app sends telemetry straight to Interfere, which some ad-blockers filter. If that's a concern, run it behind a small server and add the proxy route from [Server-side rendering](#server-side-rendering); requests then go through your own domain.
  </Accordion>
